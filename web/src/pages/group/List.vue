          ...
          <el-tab-pane label="认证方式" name="authtype">
            <el-form-item label="认证" prop="authtype">
              <el-radio-group v-model="ruleForm.auth.type" @change="authTypeChange">
                ...
                <el-radio label="oidc" border>OIDC</el-radio>
              </el-radio-group>
            </el-form-item>
            ...
            <template v-if="ruleForm.auth.type == 'oidc'">
              <el-form-item label="Issuer URL" prop="auth.oidc.issuer_url"
                            :rules="this.ruleForm.auth.type== 'oidc' ? this.rules['auth.oidc.issuer_url'] : [{ required: false }]">
                <el-input v-model="ruleForm.auth.oidc.issuer_url" 
                          placeholder="例如 https://login.microsoftonline.com/your-tenant-id/v2.0"></el-input>
              </el-form-item>
              <el-form-item label="Client ID" prop="auth.oidc.client_id"
                            :rules="this.ruleForm.auth.type== 'oidc' ? this.rules['auth.oidc.client_id'] : [{ required: false }]">
                <el-input v-model="ruleForm.auth.oidc.client_id" 
                          placeholder="从 OIDC 提供商获取的客户端 ID"></el-input>
              </el-form-item>
              <el-form-item label="Client Secret" prop="auth.oidc.client_secret"
                            :rules="this.ruleForm.auth.type== 'oidc' ? this.rules['auth.oidc.client_secret'] : [{ required: false }]">
                <el-input type="password" v-model="ruleForm.auth.oidc.client_secret" 
                          placeholder="从 OIDC 提供商获取的客户端密钥" show-password></el-input>
              </el-form-item>
              <el-form-item label="Redirect URI" prop="auth.oidc.redirect_uri"
                            :rules="this.ruleForm.auth.type== 'oidc' ? this.rules['auth.oidc.redirect_uri'] : [{ required: false }]">
                <el-input v-model="ruleForm.auth.oidc.redirect_uri" 
                          placeholder="例如 https://your-server.com/oidc/callback"></el-input>
              </el-form-item>
              <el-form-item label="Scopes" prop="auth.oidc.scopes">
                <el-input v-model="ruleForm.auth.oidc.scopes" 
                          placeholder="例如 openid profile email（留空使用默认值）"></el-input>
              </el-form-item>
              <el-form-item label="用户名字段" prop="auth.oidc.username_claim">
                <el-input v-model="ruleForm.auth.oidc.username_claim" 
                          placeholder="例如 preferred_username 或 email（留空使用默认值）"></el-input>
              </el-form-item>
              <el-form-item label="邮箱字段" prop="auth.oidc.email_claim">
                <el-input v-model="ruleForm.auth.oidc.email_claim" 
                          placeholder="例如 email（留空使用默认值）"></el-input>
              </el-form-item>
              <el-form-item label="组字段" prop="auth.oidc.groups_claim">
                <el-input v-model="ruleForm.auth.oidc.groups_claim" 
                          placeholder="例如 groups 或 roles（选填）"></el-input>
              </el-form-item>
              <el-form-item label="允许的组" prop="auth.oidc.allowed_groups">
                <el-input v-model="ruleForm.auth.oidc.allowed_groups" 
                          placeholder="用逗号分隔，例如 vpn-users,admin（留空允许所有用户）"></el-input>
              </el-form-item>
              <el-row class="msg-info">
                <el-col :span="24">
                  <h4>OIDC 配置说明：</h4>
                  <p>1. 在 OIDC 提供商（如 Azure AD、Google、Keycloak）中创建应用</p>
                  <p>2. 设置回调 URL 为：{{oidcCallbackUrl}}</p>
                  <p>3. 获取 Client ID 和 Client Secret</p>
                  <p>4. 用户选择此组后，客户端会自动打开浏览器进行认证</p>
                  <p>5. 支持的提供商：Azure AD、Google、Keycloak、Auth0 等</p>
                </el-col>
              </el-row>
            </template>
          </el-tab-pane>
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        "auth.oidc.redirect_uri": [
          {required: true, message: '请输入 Redirect URI', trigger: 'blur'}
        ],
      },
    }
  },
  methods: {
    ...
    setAuthData(row) {
      ...
      if (row.auth.type == "oidc" && !row.auth.oidc) {
        row.auth.oidc = JSON.parse(JSON.stringify(this.defAuth.oidc));
      }
